...
(defun package-diff (package1 package2 &key
  ...
  (flet ((next (head list)
    ...
    (next `((:symbol :status "Imported/excluded from" :type)
          ...
          (append (mapcar #'added-entry   (package-symbols-difference package2 package1))
                  (mapcar #'removed-entry (package-symbols-difference package1 package2))))
    (format stream "~%~%# Modified symbols~%")
    (cnext `((:symbol "Old package" :type "New package" :type)
            ,(make-list 5 :initial-element *md-align-left*))
           (mapcar #'changed-entry  (package-symbols-intersection package1 package2)))))
